...
const Logo = ({ tintColor }) => {
  const [containerImageWidth] = useState(new Animated.Value(styles.$largeContainerSize));
  ...
  const imageStyles = [styles.image, { width: imageWidth }, tintColor ? { tintColor } : null];
  return (
